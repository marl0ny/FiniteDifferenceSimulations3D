...
void init_view_programs() {
    ...
    s_view_programs.view_electric
        = make_quad_program("./shaders/view-electric.frag");
}
...
void render(const struct RenderParams *render_params) {
    glViewport(0, 0,
               s_sim_params.tex_dimensions.width_2d,
               s_sim_params.tex_dimensions.height_2d);
    step(&s_sim_frames, &s_sim_programs, &s_sim_params);
    view_of(s_view_frames.sub_views[0], s_sim_frames.electric[0],
            4, 4);
    glViewport(0, 0,
               s_view_params.view_width,
               s_view_params.view_height);
    bind_quad(s_view_frames.main_view, s_view_programs.view_electric);
    set_int_uniform("component", 2);
    set_sampler2D_uniform("tex", s_view_frames.sub_views[0]);
    draw_unbind_quad();
